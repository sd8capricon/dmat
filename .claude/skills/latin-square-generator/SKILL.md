...
reads coherently against the rendered grid. Also confirm the pacing timer's
`data-total-seconds` attribute equals `<total items> &times; 75` (e.g.
`grep -o 'data-total-seconds="[0-9]*"' <filename>.html`).
...
- **Cumulative pacing timer, 75s/question**: every generated file shows a
  ...
  byte-identical into this script and `math-equation-generator`'s (same
